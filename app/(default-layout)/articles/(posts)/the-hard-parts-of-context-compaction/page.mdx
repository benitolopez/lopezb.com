...
First, the record is appended to <CodeTag>session.jsonl</CodeTag>. Along with the summary, it stores the ID of the conversation record where the tail kept verbatim begins. In this way, compaction adds to the history without rewriting it and records a pointer into the existing chain. Only the reconstruction changes.
